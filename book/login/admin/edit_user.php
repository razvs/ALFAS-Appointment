...
if($_FILES["upload_profile"]["error"] != 4){
	...
	if(empty($email_err) && empty($mobile_number_err)){
		$update = new Staff;
		$update->update_staff_member($edit_id,$profile_img,$firstname,$lastname,$email,$mobile_number,$aboutme);
	}
	if(!empty($new_password)){
		if(empty($new_password_err) && empty($confirm_password_err)){
			$update_pw = new Staff;
			$update_pw->update_staff_member_pw($edit_id,$pw);
		}
	}		
}
?>
...
<div class="tab-panel">
    <ul class="nav nav-tabs" id="edit_user_tab">
        <li class="active"><a href="edit_user.php"><i class="glyphicon glyphicon-th-list"></i>Details</a></li>
